...
const getItem = async (TableName, Key) => {
    ...
    console.log("Elemento recuperato:", result.Item);
    if (!result.Item) {
    ...
    const deleteItem = async (TableName, Key) => {
        ...
        const result = await docClient.send(command);
        console.log("Elemento eliminato con successo:", result);
    };
    ...
    const putMetadata = async (tablename, item, partitionKeyName) => {
      const params = {
        TableName: tablename,
        Item: item,
        ConditionExpression: 'attribute_not_exists(notificationStatusTimestamp) OR #notificationStatusTimestamp < :statusChangeTimestamp',
      ...
      try {
        ...
        const result = await docClient.send(command);
        console.log("Elemento inserito con successo:", result);
      } catch (error) {
        if (error.name === 'ConditionalCheckFailedException') {
          console.log(`update not necessary for item with key: ${item[partitionKeyName]} in table: ${tablename}`);
        } else {
          throw error;
        }
      }

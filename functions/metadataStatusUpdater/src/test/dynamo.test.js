...
describe('dynamo.js tests', () => {
  ...
  describe('buildNotificationMetadataUpdateParams', () => {
    ...
    it('should include all notification metadata fields in UpdateExpression', () => {
      ...
      expect(expr).to.include('#rootSenderId = :rootSenderId');
    ...
    it('should map every field value into ExpressionAttributeValues', () => {
      ...
      expect(vals[':rootSenderId']).to.equal('rootSender1');
      ...
      expect(vals[':communicationType']).to.equal('LEGAL');
    ...
    it('should set null for optional field when it is null', () => {
      const params = buildNotificationMetadataUpdateParams('testTable', { ...fullItem, notificationGroup: null });
      expect(params.ExpressionAttributeValues[':notificationGroup']).to.be.null;
    });

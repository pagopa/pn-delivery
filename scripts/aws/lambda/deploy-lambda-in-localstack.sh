...
stream_arn=$(aws --endpoint-url=$endpoint_url --region $region dynamodb describe-table --table-name Notifications | grep -o '"LatestStreamArn": "[^"]*' | grep -o '[^"]*$')
...
then
    aws lambda delete-function --endpoint-url=$endpoint_url --region $region --function-name pn-delivery-insert-trigger
else
...
# creo la lambda
aws lambda create-function --endpoint-url=$endpoint_url --region $region --function-name pn-delivery-insert-trigger --zip-file fileb://./lambda.zip --handler index.handler --environment "Variables={QUEUE_URL=http://localstack:4566/000000000000/local-delivery-push-inputs.fifo,ENV=LOCAL}" --runtime 'nodejs16.x' --role a
# creo lo stream

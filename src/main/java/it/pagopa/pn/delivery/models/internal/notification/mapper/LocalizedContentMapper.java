...
public class LocalizedContentMapper {
    ...
    /**     * Converte un oggetto {@code LocalizedContent} proveniente dal client Data Vault     * nel corrispondente DTO server-side utilizzato dall'applicazione.     *     * <p>     * Se il parametro in input è {@code null}, il metodo restituisce {@code null}.     * I campi obbligatori del contenuto localizzato vengono copiati nel DTO target.     * </p>     *     * @param source contenuto localizzato proveniente dal client Data Vault     * @return il contenuto localizzato convertito nel formato server-side,     *         oppure {@code null} se il parametro in input è {@code null}     * @throws NullPointerException se uno dei campi obbligatori del contenuto sorgente è assente     */

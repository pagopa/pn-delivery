...
public class NotificationMetadataEntityDaoDynamo extends AbstractDynamoKeyValueStore<NotificationMetadataEntity> implements NotificationMetadataEntityDao {
    ...
    public PageSearchTrunk<NotificationMetadataEntity> searchByIun(
        ...
        // applico i filtri
        // filtro per stato
        if (!CollectionUtils.isEmpty(inputSearchNotificationDto.getStatuses()) && !inputSearchNotificationDto.getStatuses().contains(NotificationStatusV28.fromValue(entity.getNotificationStatus())))
        {
    ...
    private void addStatusFilterExpression(List<NotificationStatusV28> statuses,
                                           Expression.Builder filterExpressionBuilder,
                                           StringBuilder expressionBuilder) {
        if (!CollectionUtils.isEmpty(statuses)) {
            ...
            for (int i = 0;i<statuses.size();i++) {
                NotificationStatusV28 notificationStatus = statuses.get(i);

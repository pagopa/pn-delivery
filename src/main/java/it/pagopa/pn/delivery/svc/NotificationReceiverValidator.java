...
public class NotificationReceiverValidator {
    ...
    public void checkNewNotificationRequestBeforeInsertAndThrow(NewNotificationRequestV26 newNotificationRequestV2, String paId) {
        Set<ConstraintViolation<NewNotificationRequestV26>> errors = checkNewNotificationRequestBeforeInsert(newNotificationRequestV2, paId);
    ...
    private void checkSenderTaxIdCongruence(NewNotificationRequestV26 newNotificationRequestV2, String paId){
    ...
    protected Set<ConstraintViolation<NewNotificationRequestV26>> checkNewNotificationRequestBeforeInsert(NewNotificationRequestV26 newNotificationRequestV26, String paId) {
        Set<ConstraintViolation<NewNotificationRequestV26>> errors = new HashSet<>();

        boolean physicalAddressLookup = checkPhysicalAddressLookupIsEnabled(paId);

        // check del numero massimo di documenti allegati
        if (pnDeliveryConfigs.getMaxAttachmentsCount() > 0 && newNotificationRequestV26.getDocuments().size() > pnDeliveryConfigs.getMaxAttachmentsCount()) {
            ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("Max attachment count reached");
            errors.add(constraintViolation);
            return errors;
        }

        // check del numero massimo di recipient
        if (pnDeliveryConfigs.getMaxRecipientsCount() > 0 && newNotificationRequestV26.getRecipients().size() > pnDeliveryConfigs.getMaxRecipientsCount()) {
            ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("Max recipient count reached");
            errors.add(constraintViolation);
            return errors;
        }

        // check campi paFee e vat per notifiche DELIVERY_MODE
        if ( newNotificationRequestV26.getNotificationFeePolicy().equals(NotificationFeePolicy.DELIVERY_MODE) &&
                (Objects.isNull(newNotificationRequestV26.getPaFee()) || Objects.isNull(newNotificationRequestV26.getVat()))) {
            ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("paFee or vat field not filled in for notification with notificationFeePolicy DELIVERY_MODE");
            errors.add( constraintViolation );
        }
        ...
        for (NotificationRecipientV24 recipient : newNotificationRequestV26.getRecipients()) {
            ...
            boolean isNotificationFeePolicyDeliveryMode = newNotificationRequestV26.getNotificationFeePolicy().equals(NotificationFeePolicy.DELIVERY_MODE);
        ...
        if (!hasDistinctAttachments(newNotificationRequestV26)) {
    ...
    private Set <ConstraintViolation<NewNotificationRequestV26>> checkPaymentAttachmentExtension(List<NotificationPaymentItem> payments) {
        Set<ConstraintViolation<NewNotificationRequestV26>> violations = new HashSet<>();
    ...
    private void checkContentType(String contentType, String key, Set<ConstraintViolation<NewNotificationRequestV26>> violations) {
        if (APPLICATION_PDF_CONTENT_TYPE.equalsIgnoreCase(contentType) && (!key.contains(PN_NOTIFICATION_ATTACHMENTS) || key.endsWith(EXTENSION_JSON) )) {
            ConstraintViolationImpl<NewNotificationRequestV26> violation = new ConstraintViolationImpl<>(String.format("Key: %s does not conform to the expected content type: %s", key, contentType));
     ...
     * @return
     */
    protected Set<ConstraintViolation<NewNotificationRequestV26>> checkTaxId(NotificationRecipientV24 recipient, Set<String> distinctTaxIds, int recIdx){
        ...
        if( !validateUtils.validate(recipient.getTaxId(), isPF, false, skipCheckTaxIdInBlackList)) {
            ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>( "Invalid taxId for recipient " + recIdx );
            errors.add(constraintViolation);
        }else{
        ...
        if ( !distinctTaxIds.add( recipient.getTaxId() )){
            ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>( "Duplicated recipient taxId" );
            errors.add(constraintViolation);
        }
    ...
    /**
     * Validazione di NewNotificationRequestV26 per verificare l'assenza di duplicati tra gli allegati
     * @param newNotificationRequest
     * @return
     */
    protected boolean hasDistinctAttachments(NewNotificationRequestV26 newNotificationRequest){
    ...
    protected Set<ConstraintViolation<NewNotificationRequestV26>> checkPhysicalAddress(NotificationPhysicalAddress physicalAddress, int recIdx) {

        Set<ConstraintViolation<NewNotificationRequestV26>> errors = new HashSet<>();

        if(physicalAddress == null){
            ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("PhysicalAddress cannot be null");
            errors.add(constraintViolation);
            return errors;
        }
        ...
        if (this.pnDeliveryConfigs.isPhysicalAddressValidation()) {
            ...
            Stream.of(address, addressDetails, province, foreignState, at, zip, municipality, municipalityDetails)
                    ...
                    .map(field -> new ConstraintViolationImpl<NewNotificationRequestV26>(String.format("Field %s in recipient %s contains invalid characters.", field.getKey(), recIdx)))
                    .forEach(errors::add);
            ...
            Stream.of(row2, addressDetails, address, row5, foreignState)
                    ...
                    .map(field -> new ConstraintViolationImpl<NewNotificationRequestV26>(String.format("Field %s in recipient %s exceed max length of %s chars", field.getKey(), recIdx, this.pnDeliveryConfigs.getPhysicalAddressValidationLength())))
                    .forEach(errors::add);
    ...
    protected Set<ConstraintViolation<NewNotificationRequestV26>> checkDenomination(NotificationRecipientV24 recipient, int recIdx) {
        ...
        if(this.pnDeliveryConfigs.getDenominationLength() != null && this.pnDeliveryConfigs.getDenominationLength() != 0){
            fieldsToCheck.stream()
                    ...
                    .map(field -> new ConstraintViolationImpl<NewNotificationRequestV26>(String.format("Field %s in recipient %s exceed max length of %s chars", field.getKey(), recIdx, this.pnDeliveryConfigs.getDenominationLength())))
                    .forEach(errors::add);
        }
    ...
    private Set<ConstraintViolation<NewNotificationRequestV26>> checkApplyCost(boolean isNotificationFeePolicyDeliveryMode, List<NotificationPaymentItem> payments){
        ...
        for (NotificationPaymentItem paymentInfo : payments) {
            ...
            if(paymentInfo.getF24() != null) {
                ...
                if(!StringUtils.hasText(paymentInfo.getF24().getTitle())){
                    ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("F24 description is mandatory");
                    errors.add(constraintViolation);
                }
            }
        }
    ...
    private void checkApplyCost(int pagoPAApplyCostFlgCount, int f24ApplyCostFlgCount, boolean notificationHasPagoPaPayments, boolean notificationHasF24Payments, boolean isNotificationFeePolicyDeliveryMode, Set<ConstraintViolation<NewNotificationRequestV26>> errors) {
        if (isNotificationFeePolicyDeliveryMode) {
            if (notificationHasPagoPaPayments && pagoPAApplyCostFlgCount == 0) {
                ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("PagoPA applyCostFlg must be valorized for at least one payment");
                errors.add(constraintViolation);
            }
            if (notificationHasF24Payments && f24ApplyCostFlgCount == 0) {
                ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("F24 applyCostFlg must be valorized for at least one payment");
                errors.add(constraintViolation);
            }
        } else {
            if (pagoPAApplyCostFlgCount != 0) {
                ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("PagoPA applyCostFlg must not be valorized for any payment");
                errors.add(constraintViolation);
            }
            if (f24ApplyCostFlgCount != 0) {
                ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("F24 applyCostFlg must not be valorized for any payment");
                errors.add(constraintViolation);
            }
        }
    }

    public Set<ConstraintViolation<NewNotificationRequestV26>> checkIuvs(List<NotificationPaymentItem> payments, Set<String> iuvSet, int recIdx) {
        Set<ConstraintViolation<NewNotificationRequestV26>> errors = new HashSet<>();
        int paymIdx = 0;
        for (NotificationPaymentItem payment : payments) {
            if(payment.getPagoPa() != null) {
                ...
                if ( !iuvSet.add( iuv ) ) {
                    ...
                    ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>(errorMsg);
                    errors.add(constraintViolation);
                }
            }
    ...
    private static void onlyNumericalTaxIdForPGV2(Set<ConstraintViolation<NewNotificationRequestV26>> errors, int recIdx, NotificationRecipientV24 recipient) {
        if (NotificationRecipientV24.RecipientTypeEnum.PG.equals(recipient.getRecipientType()) &&
                (!recipient.getTaxId().matches("^\\d+$"))) {
            ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("SEND accepts only numerical taxId for PG recipient " + recIdx);
            errors.add(constraintViolation);
        }
    }
    ...
    private static void checkProvinceV2(Set<ConstraintViolation<NewNotificationRequestV26>> errors, NotificationPhysicalAddress physicalAddress) {
        if (Objects.nonNull(physicalAddress) &&
            ...
            ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("No province provided in physical address");
            errors.add(constraintViolation);
        }
    }
    ...
    public Set<ConstraintViolation<NewNotificationRequestV26>> checkNewNotificationRequestForMVP(NewNotificationRequestV26 newNotificationRequestV2) {
        Set<ConstraintViolation<NewNotificationRequestV26>> errors = new HashSet<>();

        if (newNotificationRequestV2.getRecipients().size() > 1) {
            ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("Max one recipient");
            errors.add(constraintViolation);
        }
        ...
        if (Objects.isNull(payment) || payment.isEmpty()) {
            ConstraintViolationImpl<NewNotificationRequestV26> constraintViolation = new ConstraintViolationImpl<>("No recipient payment");
            errors.add(constraintViolation);
        }
        return errors;
    }

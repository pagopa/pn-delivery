...
class PnResponseEntityExceptionHandlerActivationTest {
    ...
    void handleIoMandateNotFoundException() {
        ...
        ResponseEntity<CheckQrForbiddenResponse> response = handler.handleIoMandateNotFoundException(exception);
        assertNotNull(response);
        assertEquals(403, response.getStatusCodeValue());
        assertNotNull(response.getBody());
        assertEquals(CheckQrForbiddenResponse.ErrorTypeEnum.MANDATE_NOT_PRESENT, response.getBody().getErrorType());
    }
